...
from models import db, User, Room, Review, Favorite
...
def index():
    ...
    if keyword:
        query = query.filter(
            Room.title.ilike(f"%{keyword}%") | Room.description.ilike(f"%{keyword}%")
        )
...
def room_detail(room_id):
    ...
    if request.method == "POST":
        ...
        if content:
            new_review = Review(
                content=content, user_id=current_user.id, room_id=room.id
            )
    ...
    if current_user.is_authenticated:
        is_favorited = (
            Favorite.query.filter_by(user_id=current_user.id, room_id=room.id).first()
            is not None
        )

    return render_template(
        "room_detail.html", room=room, reviews=reviews, is_favorited=is_favorited
    )
...
def my_rooms():
    ...
    rooms = (
        Room.query.filter_by(chutro_id=current_user.id).order_by(Room.id.desc()).all()
    )
...
@app.route("/favorite/<int:room_id>", methods=["POST"])
@login_required
def toggle_favorite(room_id):
    ...
    if favorite:
        ...
        new_favorite = Favorite(user_id=current_user.id, room_id=room.id)
        db.session.add(new_favorite)
        flash("Room added to favorites.", "success")

    db.session.commit()

    return redirect(request.referrer or url_for("room_detail", room_id=room.id))
